...
class CategoryArticleModel extends AdminModel
{
    ...
    protected $fillable            = ['status', 'name','status','ordering'];
    ...
    {
        return $this->hasMany(CategoryArticleModel::class, 'parent_id', 'id')->with('categoriesArticle');
    }

    public function categoriesProductActive()
    {
        return $this->hasMany(CategoryArticleModel::class, 'parent_id', 'id')->where('status', 'active')->with('categoriesArticleActive');
    }
    ...
    public function listItems($params = null, $options = null)
    {
        ...
        if ($options['task'] == "admin-list-items") {
            $query = $this->select('id', 'name', 'status', 'ordering', 'created', 'created_by', 'modified', 'modified_by', 'parent_id')->whereNull('parent_id');
    ...
    public function getItem($params = null, $options = null)
    {
        ...
        if ($options['task'] == 'get-item') {
            $result = self::select('id', 'name', 'status','ordering', 'parent_id','_lft','_rgt')->where('id', $params['id'])->first();
        }

        if ($options['task'] == 'store-get-item') {
            $result = self::select('id', 'name')->where('id', $params['category_article_id'])->first();
